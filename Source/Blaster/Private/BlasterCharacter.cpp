...
void ABlasterCharacter::AimOffset(float DeltaTime)
{
	...
	if (Speed == 0.f && !bIsinAir) // not running AND not falling settings
	{
	...
	if (Speed > 0.f || bIsinAir) // running OR jumping settings
	{
...
void ABlasterCharacter::TurnInPlace(float DeltaTime)
{
	...
	if (TurningInPlace != ETurningInPlace::ETIP_NotTurning)
	{
		InterpAO_Yaw = FMath::FInterpTo(InterpAO_Yaw, 0.f, DeltaTime, 3.3f);
		AO_Yaw = InterpAO_Yaw;
